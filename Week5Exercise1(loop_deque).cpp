...
struct userDeque {
	...
	inline void insertDeque(long num) {
		if (len == dequeSize) {
			...
			memcpy(tmp, element + head, sizeof(long) * (len - head));
			memcpy(tmp + len - head, element, sizeof(long) * head);
			delete[] element;
			element = tmp, head = 0;
		}
		element[(head + len) % dequeSize] = num, len++;
	}
	inline long deleteDeque(long flag) {
		if (len == 0) return -1;
		else {
			(flag) ? (len--) : (head = (head + 1) % dequeSize, len--); return 1;
		}
	}
	inline void printDeque() {
		if (!len) printf("%s\n", "NULL");
		else {
			for (long i = 0; i < len; i++) printf("%ld ", element[(head + i) % dequeSize]);
			printf("\n");
		}
	}
};
